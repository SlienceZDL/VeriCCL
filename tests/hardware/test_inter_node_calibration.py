...
def test_inter_node_128_mib_calibration_xmls():
    config = hardware_config(minimum_gpu_count=2, require_hostfile=True)
    directory = xml_directory("VERICCL_INTER_CALIBRATION_XML_DIR")
    xmls = tuple(sorted(directory.glob("*.xml")))
    if not xmls:
        pytest.skip("VERICCL_INTER_CALIBRATION_XML_DIR contains no XML files")
    artifacts = tuple(
        sorted(
            (
