...
Calibration uses exactly 128 MiB and the input slice size. The slice size must divide 128 MiB or calibration is `not_run`. The representative Broadcast benchmark is in-place so sender traces measure link transfer rather than root-side input-to-output copies. Both intra-node and inter-node calibration launch one MPI process per GPU with `-g 1`; only inter-node execution additionally requires a hostfile.
...
Keep the documented `--timeout-s 10800` for an uncached 16-point calibration. Each calibration point starts one correctness-enabled release process with five warmups and 20 measurements, followed by one 20-iteration trace process. Each trace sample uses the aggregate sender-local elapsed interval normalized by the number of complete waves, so independently advancing channel TBs do not need lockstep wave boundaries. The trace iterations determine calibration stability and `B_link(k)`. Final operator validation remains stricter: it uses rounds of 20 independent release processes, with up to three rounds when measurements are unstable.
