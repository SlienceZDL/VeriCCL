...
class Algorithm(object):
    def __init__(self, name, collective, topology, instance, stages, steps, input_map = {}, output_map = {}, cont=False):
        self.name = name
        self.topology = topology
        self.collective = collective
        self.instance = instance
        self.stages = stages # ADD
        self.steps = steps
        self.input_map = input_map
        self.output_map = output_map
        self.cont = cont
        
        # TACCL schunk bug: 1.self.input_map 2.self.output_map
        if self.collective.is_combining == True:
            input_map = {}
            output_map = {}
            for i in range(self.topology.ngpus):
                input_addrs = set()
                output_addrs = set()
                for j in range(self.instance.chunks):
                    input_addrs.add(j)
                    output_addrs.add(j)
                input_map[i]= input_addrs
                output_map[i] = output_addrs
            self.output_map = output_map
            self.input_map = input_map
        else:
            self.input_map = copy.copy(input_map) if input_map is not None else {}
            self.output_map = copy.copy(output_map) if output_map is not None else {}
    ...
    @classmethod
    def make_implementation(cls, collective, topology, instance, stages, steps, cont=False, suffix=""):
        ...
        algo = cls(name, collective, topology, instance, stages, steps, input_map, output_map, cont)
    ...
    def check_implements(self, collective):
        if self.topology.num_nodes() != collective.num_nodes: # 16
        ...
        # for r, rank in enumerate(collective.ranks()):
        #     for c, chunk in enumerate(collective.chunks()):
        #         if collective.postcondition(rank, c) and not state[idx(r, c)]:
