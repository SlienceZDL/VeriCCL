    ...
    def handle(args, command):
        ...
        def rs_chunk_stage1(algo_json):
            ...
            for i in range(len(steps)):
                ...
                new_step4 = copy.deepcopy(steps[i]) # 操作之前复制
                new_step5 = copy.deepcopy(steps[i]) # 操作之前复制
                new_step6 = copy.deepcopy(steps[i]) # 操作之前复制
                new_step7 = copy.deepcopy(steps[i]) # 操作之前复制

                # 对原先的step的每一个send的第一个chunk_id进行×2
                sends =  steps[i]["sends"]
                for j in range(len(sends)):
                    sends[j][0] = sends[j][0] * 8 # 或者是4

                # 对拷贝的step的每一个send的第一个chunk_id进行×2 + 1
                new_sends1 =  new_step1["sends"]
                for j in range(len(new_sends1)):
                    new_sends1[j][0] = new_sends1[j][0] * 8 + 1

                new_sends2 =  new_step2["sends"]
                for j in range(len(new_sends2)):
                    new_sends2[j][0] = new_sends2[j][0] * 8 + 2

                new_sends3 =  new_step3["sends"]
                for j in range(len(new_sends3)):
                    new_sends3[j][0] = new_sends3[j][0] * 8 + 3
                    
                new_sends4 =  new_step4["sends"]
                for j in range(len(new_sends4)):
                    new_sends4[j][0] = new_sends4[j][0] * 8 + 4

                new_sends5 =  new_step5["sends"]
                for j in range(len(new_sends5)):
                    new_sends5[j][0] = new_sends5[j][0] * 8 + 5

                new_sends6 =  new_step6["sends"]
                for j in range(len(new_sends6)):
                    new_sends6[j][0] = new_sends6[j][0] * 8 + 6
                
                new_sends7 =  new_step7["sends"]
                for j in range(len(new_sends7)):
                    new_sends7[j][0] = new_sends7[j][0] * 8 + 7
                ...
                new_steps.append(new_step5)
                new_steps.append(new_step6)
                        ...
                        if index == 0 or index == 3:
                            ...
                            for step in steps:
                                ...
                                for send in sends: # 更新 rank id , chunk id -> i 从 1 开始, +的2是 16 / 8
                                    send[0] = send[0] + i * 8 # 2表示reducescatter的分组数量
                                    send[1] = send[1] + step_len * i
                                    send[2] = send[2] + step_len * i
                            new_algo_name = str(reduction[0]) + '_'.join(map(str, group))
                            algo_jsons[new_algo_name] = json.dumps(new_algo_json)
