...
def handle_write_to_directory(directory, force, get_contents, preferred_file_name):
    output_file = directory / preferred_file_name
    if output_file.exists():
        if output_file.is_dir():
            print(f'error: output path is a directory', file=sys.stderr)
...
def parse_and_get_topo(node_topology: NodeTopology, comm_sketch_file, reduce=False):
    ...
    print(F"comm_sketch_file:{comm_sketch_file}")
...
def parse_and_get_topo_(node_topology: NodeTopology, comm_sketch_file, reduce=False):

    print(F"comm_sketch_file:{comm_sketch_file}")

    file = open(comm_sketch_file, 'r')
