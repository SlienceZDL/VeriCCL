...
class TACCLRouting:
    ...
    def _encode(self, opt):
        ...
        # 优化目标 MINIMIZE self.time

        # 变量有 self.time：连续变量，表示整个通信完成的时间（目标优化量）
        # self.send[c,src,dst,l]：连续变量，表示数据块c从src节点通过链路l发送到dst节点的时间点
        # self.start[c,r]：连续变量，表示数据块c在节点r上开始处理的时间点
        # self.is_sent[c,src,dst,l]：二元变量，表示数据块c是否从src通过链路l发送到dst
        ...
        if self.topology.copies > 1:
            ...
            if self.route_sketch.internode.enforce_ordering: # modify
    ...
    def optimize_(self, distribute_over_links):
        import pickle as pkl
        heuristic = self.route_sketch.hyperparameters.heuristic
        # print("HEURISTIC:", heuristic)
        # print("finding shortest path sets")
