...
def _is_relay_link(topology, src, dst):
    if "copies" in topology.name:
        ...
        copies = 1
    # num_local = len(topology.links) // topology.copies
...
def _allocate_channels_match_stage(op_sets, stage_list, logging):
    
    def get_stage_id(step_idx):
        for s, stage in enumerate(stage_list):
            if step_idx in stage:
                return s
    ...
    for op_set in op_sets:
        send = op_set[0]
        stage_id = get_stage_id(send.step)
        assert send.op_type == 's'
        ops_by_channel[stage_id].extend(op_set)
...
def ncclize(algorithm, remap_scratch = None, channel_policy=ChannelPolicy.MatchTopology, pretty_print = True, old_format=False, use_scratch=False, merge_contiguous=True, instances=1, scale_remote=1, combine_contig=False, aid_IB_contig=False, prefix="", logging=False):
    ...
    if "Allreduce" in algorithm.name:
        # print(algorithm.input_map, type(algorithm.input_map))
        # 修改input output
        chunks = algorithm.instance.chunks
        algorithm.input_map = {rank : [chunk for chunk in range(chunks)] for rank in algorithm.ranks() }
        algorithm.output_map = {rank : [chunk for chunk in range(chunks)] for rank in algorithm.ranks() }

    # return

    # Create GPUs, their address to buffer mappings and possible copies
    gpus = {}
    for rank in algorithm.ranks(): # 16
        outputs = {}
    ...
    for key, grp in tb_groups.items():
        ...
        for tb in tbs:
            ...
            if tb_peer == -1 or tb_peer == peer:
                if tb.recv != -1 and tb.send != -1: # ADD
                # (tb.recv != peer) and (tb.send != peer) # ADD
    ...
    if old_format:
        ...
        if "Allgather" in algorithm.name:
            ...
            algo_elem.set('inplace', "1")
        # ADD
        elif "Reduce" in algorithm.name:
            algo_elem.set('coll', "reduce")
            algo_elem.set('inplace', "1")
        elif "Broadcast" in algorithm.name:
            algo_elem.set('coll', "broadcast")
            algo_elem.set('inplace', "1")
        algo_elem.set('redop', "nop")
        algo_elem.set('ngpus', str(len(gpus)))
        algo_elem.set('maxBytes', str(4294967296))
    for rank, gpu in gpus.items():
...
def ncclize_pipeline(algorithm, remap_scratch = None, channel_policy=ChannelPolicy.MatchStage, pretty_print = True, old_format=False, use_scratch=False, merge_contiguous=True, instances=1, scale_remote=1, combine_contig=False, aid_IB_contig=False, prefix="", logging=False):
    '''
    Generate the XML format used by the NCCL SCCL backend.
    ...
    if algorithm.is_pipelined():
        print("Generate pipeline algorithm...")
    ...
    for step_idx, step in enumerate(algorithm.steps):
        ...
        stage_id = get_stage_id(step_idx)
            ...
            for (src, dst), addrs in grouped_sends.items():
                for src_buf, src_off, dst_buf, dst_off, cnt in make_intervals(src, dst, addrs):
                    for i in range(instances):
                        new_src_off = src_off * instances + i * cnt
                        new_dst_off = dst_off * instances + i * cnt
                        send = (src, dst, src_buf, new_src_off, dst_buf, new_dst_off, cnt, stage_id)
                        sends.append(send)
        elif len(step.sends[0])==6:
            for (src,dst,t,l,redop) in sorted(grouped_sends, key=lambda x: x[2]):
                addrs = grouped_sends[(src,dst,t,l,redop)]
                for src_buf, src_off, dst_buf, dst_off, cnt in make_intervals(src, dst, addrs):
                    for i in range(instances):
                        new_src_off = src_off * instances + i * cnt
                        new_dst_off = dst_off * instances + i * cnt
                        send = (src, dst, src_buf, new_src_off, dst_buf, new_dst_off, cnt, redop, stage_id)
                        sends.append(send)
        else:
            for (src, dst,t,l) in sorted(grouped_sends, key=lambda x: x[2]):
                addrs = grouped_sends[(src,dst,t,l)]
        ...
        for send in sends:
            redop = None
            if len(send) == 8:
                src, dst, src_buf, src_off, dst_buf, dst_off, cnt, stage_id = send
            else:
                src, dst, src_buf, src_off, dst_buf, dst_off, cnt, redop, stage_id = send
            read_keys = [(src,src_buf,src_off+i) for i in range(cnt)]
            # A send must wait for the previous recv (if any) to finish
            send_depends = list(set(d for k in read_keys for d in writers[k]))
            ...
            send_op = _Op(src, dst, step_idx, True, 's', src_buf, src_off, dst_buf, dst_off, cnt, send_depends, stage_id=stage_id)
            if redop is None:
                recv_op = _Op(dst, src, step_idx, False, 'r', src_buf, src_off, dst_buf, dst_off, cnt, recv_depends, stage_id=stage_id)
            else:
                assert redop == 'rrc'
                is_reduce = True
                recv_op = _Op(dst, src, step_idx, False, redop, src_buf, src_off, dst_buf, dst_off, cnt, recv_depends, stage_id=stage_id)
        ...
        for key, deps in new_writers.items():
            if key in new_readers:
                ...
                if "phasewise" in prefix:
                    ...
                    deplist = old_recv_op.depends
                    deplist.append(dep_send_op)
                    new_recv_op = _Op(old_recv_op.gpu, old_recv_op.peer, old_recv_op.step, False, old_recv_op.op_type, old_recv_op.src_buffer, old_recv_op.src_offset, old_recv_op.dst_buffer, old_recv_op.dst_offset, old_recv_op.cnt, deplist, stage_id=old_recv_op.stage_id)
                    for i, op_set in enumerate(op_sets):
                        if op_set[1] == old_recv_op:
                            op_sets[i][1] = new_recv_op
                    new_writers[key][0] = new_recv_op
    ...
    if channel_policy == ChannelPolicy.One:
        ...
        ops_by_channel = _allocate_channels_match_topology(op_sets, algorithm.topology, instances, scale_remote, logging)
    elif channel_policy == ChannelPolicy.MatchStage:
        ops_by_channel = _allocate_channels_match_stage(op_sets, stage_list, logging)
    else:
        assert False, 'Unhandled channel policy'
    ...
    # Generate the XML structure
    algo_elem = ET.Element('algo')
    algo_elem.set('name', "canvas")
    # algo_elem.set('nchannels', str(1 + max(max(tb.channel for tb in gpu.threadblocks) for gpu in gpus.values())))
    assert len(stage_list) > 0
    algo_elem.set('nchannels', str(len(stage_list)))
    if old_format:
        algo_elem.set('nchunksperloop', str(max(max(gpu.input_chunks, gpu.output_chunks) for gpu in gpus.values())))
        algo_elem.set('proto', "Simple")
        if "Allgather" in algorithm.name:
