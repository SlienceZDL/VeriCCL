...
class Collective:
    def __init__(self, name, num_nodes, chunks, ranks=None, group = None, triggers = {}):
        self.name = name
        ...
        addresses_seen = set() # 检查是否是reduce操作?
...
def build_collective_allreduce(name, group, num_nodes, num_chunks, precondition, postcondition, address = lambda c: c, trigger = lambda r, c: None):
    chunks = []
    ranks = []

    for i in group:
        chunks.append(Chunk({i}, set(group), 0))

    n = 0
    for i in range(num_nodes):
        if i in group:
            ranks.append(Rank({n}, set(range(len(group))), i))
            n += 1
        else:
            ranks.append(Rank({}, {}, i))
    ...
    return Collective(name, num_nodes, chunks, ranks, group, triggers)


def build_collective_reduce(name, group, num_nodes, num_chunks, precondition, postcondition, address = lambda c: c, trigger = lambda r, c: None):
    chunks = []
    ranks = []

    for i in group:
        chunks.append(Chunk({i}, {group[0]}, 0))
...
def build_collective_allgather(name, group, num_nodes, num_chunks, precondition, postcondition, address = lambda c: c, trigger = lambda r, c: None):
    chunks = []
    ranks = []

    for i, v in enumerate(group):
        chunks.append(Chunk({v}, set(group), i))
...
def build_collective_broadcast(name, group, num_nodes, num_chunks, precondition, postcondition, address = lambda c: c, trigger = lambda r, c: None):
    chunks = []
    ranks = []

    chunks.append(Chunk({group[0]}, set(group), 0))

    f = 0
    for i in range(num_nodes):
        if i in group:
            if f == 0:
                ranks.append(Rank({0}, {0}, i))
            else:
                ranks.append(Rank({}, {0}, i))
            f += 1
        else:
            ranks.append(Rank({}, {}, i))
...
def sub_allgather(num_nodes, group):
    return build_collective_allgather(f'Sub_Allgather(n={num_nodes})', group, num_nodes, num_nodes, _scattered(num_nodes), _all)

def sub_allreduce(num_nodes, group):
    return build_collective_allreduce(f'Sub_Allreduce(n={num_nodes})', group, num_nodes, num_nodes, _scattered(num_nodes), _all, _single_scattered(num_nodes))

def sub_reduce(num_nodes, group, root):
    return build_collective_reduce(f'Sub_Reduce(n={num_nodes},root={root})', group, num_nodes, num_nodes, _scattered(num_nodes), _root(root), _single_scattered(num_nodes))

def sub_broadcast(num_nodes, group, root):
    return build_collective_broadcast(f'Sub_Broadcast(n={num_nodes},root={root})', group, num_nodes, 1, _root(root), _all)
